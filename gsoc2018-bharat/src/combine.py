...
"""
The following snippet is used to count the unique resources in the entire corpus.
...
def reader(file_name):
	with codecs.open(file_name, 'r', encoding='utf-8', errors='ignore') as input_file:
...
with open(wiki_file, 'w+') as output_file:
	for root, directories, files in os.walk(root_directory):
		for f in files:
			...
			path = root + '/' + f
			with codecs.open(path, 'r', encoding='utf-8', errors='ignore') as input_file:
				for line in input_file:
					if '<doc' in line:
						pass
					else:
